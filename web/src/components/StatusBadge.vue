...
// Status vocabulary from the global contract, with the exact display words.
...
const tone: Record<string, string> = {
  ...
  PaymentPending: 'bg-amber-50 text-amber-800 border-amber-200',
  Incomplete: 'bg-amber-50 text-amber-800 border-amber-200',
  Offered: 'bg-sky-50 text-sky-800 border-sky-200',
  OfferedSpot: 'bg-sky-50 text-sky-800 border-sky-200',
...
<template>
  <Badge variant="outline" :class="cn('font-medium', tone[status])">{{ text }}</Badge>
</template>
