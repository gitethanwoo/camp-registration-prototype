...
interface Detail {
  id: number, status: string, grade: number, gradeLabel: string, createdAt: string
  participant: { id: number, firstName: string, lastName: string, dateOfBirth: string, gender: string, dietary: string | null, allergies: string | null, adaNeeds: string | null }
  household: {
    id: number, name: string, email: string, phone: string, city: string, salesforceId: string | null
  ...
  waivers: { title: string, currentVersion: number, status: string, acceptedVersion: number | null, signerName: string | null, acceptedAt: string | null }[]
  money: {
    priceCents: number, discountCents: number, paidCents: number, balanceCents: number, confirmationCode: string, option: string | null
    balanceDueDate: string, orderParticipants: number
    operations: { id: number, kind: string, amountCents: number, succeeded: boolean, processorRef: string, cardLast4: string | null, reason: string | null, createdAt: string }[] | null
...
async function load() { r.value = await api.get<Detail>(`/admin/registrations/${props.id}`) }
onMounted(load)

const answerLabels: Record<string, string> = { tshirt: 'T-shirt size', swim: 'Swim level', church: 'Attends church', churchName: 'Church', cabinmate: 'Cabin-mate request', bus: 'Bus from Atlanta' }
...
async function cancel() {
  if (!reason.value.trim()) { cancelError.value = 'Add a reason. It goes in the audit log.'; return }
  cancelling.value = true
  cancelError.value = null
  try {
    await api.post(`/admin/registrations/${props.id}/cancel`, { reason: reason.value, refundCents: refundCents.value })
    cancelOpen.value = false
    toast.success(`Cancelled. ${refundCents.value ? `${money(refundCents.value)} refunded to card.` : 'No refund issued.'} Seat released.`)
    await load()
  }
  catch (e) { cancelError.value = e instanceof ApiError ? e.message : 'Something went wrong.' }
    ...
    <template v-else>
      <div class="flex flex-wrap items-start gap-4">
        <div class="flex-1">
          <div class="flex flex-wrap items-center gap-3">
            <h1 class="text-2xl font-semibold tracking-tight">{{ r.participant.firstName }} {{ r.participant.lastName }}</h1>
            <StatusBadge :status="r.status" />
          </div>
          <p class="text-muted-foreground">{{ r.program.name }} · {{ r.session.name }} · {{ r.pool }}<template v-if="r.pool !== r.gradeLabel"> · {{ r.gradeLabel }}</template></p>
          <p class="text-sm text-muted-foreground">Confirmation {{ r.money.confirmationCode }} · registered {{ dateTime(r.createdAt) }}</p>
        </div>
        <AlertDialog v-if="r.cancellation" v-model:open="cancelOpen">
          <AlertDialogTrigger as-child><Button variant="outline" @click="openCancel">Cancel registration</Button></AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <div class="space-y-4">
              <div class="rounded-md border bg-muted/40 p-3 text-sm">
                ...
                <p class="text-muted-foreground">{{ r.cancellation.daysUntilStart }} days until start · paid {{ money(r.money.paidCents) }} · suggested refund {{ money(r.cancellation.suggestedRefundCents) }}</p>
              </div>
              <div class="space-y-2">
                <Label for="refund">Refund to card</Label>
                <Input id="refund" v-model="refund" inputmode="decimal" />
                <p class="text-xs text-muted-foreground">Up to {{ money(r.cancellation.maxRefundCents) }}. Overrides are allowed and logged.</p>
              </div>
              <div class="space-y-2">
                <Label for="reason">Reason</Label>
                <Textarea id="reason" v-model="reason" rows="2" placeholder="e.g. Family schedule conflict, called 9/23" />
              </div>
            ...
            <AlertDialogFooter>
              ...
              <Button variant="destructive" :disabled="cancelling" @click="cancel">
                {{ cancelling ? 'Cancelling…' : refundCents ? `Cancel and refund ${money(refundCents)}` : 'Cancel without refund' }}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      ...
      <div class="grid gap-6 xl:grid-cols-[1fr_340px]">
            ...
            <Card>
              ...
              <CardContent class="divide-y text-sm">
                <div class="flex items-center justify-between py-3 first:pt-0">
                  <div>
                    <p class="font-medium">Health form {{ r.program.healthMechanism === 'CampDoc' ? '(CampDoc)' : '' }}</p>
                    <p class="flex items-center gap-1 text-muted-foreground">
                      <template v-if="r.healthOnFile"><Lock class="size-3.5" />On file. Visible to camp health staff only.</template>
                      <template v-else-if="r.program.healthMechanism === 'CampDoc'">Status synced from CampDoc.</template>
                    </p>
                  </div>
                  <StatusBadge :status="r.healthStatus" />
                </div>
                <div v-for="w in r.waivers" :key="w.title" class="flex items-center justify-between gap-4 py-3 last:pb-0">
                  <div>
                    <p class="font-medium">{{ w.title }}</p>
                    <p class="text-muted-foreground">
                      <template v-if="w.acceptedAt">v{{ w.acceptedVersion }} signed by {{ w.signerName }} · {{ dateTime(w.acceptedAt) }}</template>
              ...
              <CardContent>
                <dl class="grid gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
                  <div v-for="(v, k) in r.answers" :key="k">
                    <dt class="text-muted-foreground">{{ answerLabels[k] ?? k }}</dt><dd>{{ v || '—' }}</dd>
                  </div>
                  <div><dt class="text-muted-foreground">Allergies</dt><dd>{{ r.participant.allergies || 'None' }}</dd></div>
                  <div><dt class="text-muted-foreground">Dietary</dt><dd>{{ r.participant.dietary || 'None' }}</dd></div>
                </dl>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="payments" class="mt-4 space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>{{ r.money.option }} · order covers {{ r.money.orderParticipants }} {{ r.money.orderParticipants === 1 ? 'camper' : 'campers' }}</CardDescription>
              </CardHeader>
              <CardContent class="space-y-1 text-sm">
                <div class="flex justify-between"><span>Price</span><span class="tabular-nums">{{ money(r.money.priceCents) }}</span></div>
                <div v-if="r.money.discountCents" class="flex justify-between"><span>Discount</span><span class="tabular-nums">−{{ money(r.money.discountCents) }}</span></div>
                <div class="flex justify-between"><span>Paid</span><span class="tabular-nums">{{ money(r.money.paidCents) }}</span></div>
                <div class="flex justify-between font-medium"><span>Balance</span><span class="tabular-nums">{{ money(r.money.balanceCents) }}</span></div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <ol class="space-y-3 text-sm">
                  <li v-for="(a, i) in r.audit" :key="i" class="border-l-2 pl-3">
                    <p>{{ a.detail }}</p>
                    <p class="text-xs text-muted-foreground">{{ a.actor }} · {{ dateTime(a.createdAt) }} · {{ a.action }}</p>
                  </li>
            ...
            <Card class="mt-4">
              ...
              <CardContent class="divide-y text-sm">
                <div v-for="(m, i) in r.messages" :key="i" class="flex justify-between py-2">
                  ...
                  <span class="text-muted-foreground">{{ m.processedAt ? `Sent ${dateTime(m.processedAt)}` : 'Queued' }}</span>
                </div>
        ...
        <aside class="space-y-4">
          <Card>
            <CardHeader><CardTitle>{{ r.household.name }} household</CardTitle><CardDescription>{{ r.household.city }}</CardDescription></CardHeader>
          ...
          <Card v-if="r.household.otherRegistrations.length">
            ...
            <CardContent class="space-y-2 text-sm">
              <RouterLink v-for="o in r.household.otherRegistrations" :key="o.id" :to="`/admin/registrations/${o.id}`" class="flex items-center justify-between gap-2 rounded-md p-1 hover:bg-muted">
                <span>{{ o.participant }} <span class="text-muted-foreground">· {{ o.program }}</span></span>
          ...
          <Card>
            ...
            <CardContent class="text-sm text-muted-foreground">{{ dateRange(r.session.startDate, r.session.endDate) }}<br>DOB {{ date(r.participant.dateOfBirth) }}</CardContent>
          </Card>
        </aside>
      </div>
    </template>
