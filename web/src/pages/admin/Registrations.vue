...
async function load() {
  loading.value = true
  try {
    data.value = await api.get<Page>(`/admin/registrations?${new URLSearchParams(params.value)}`)
  } finally {
    loading.value = false
  }
  const { sessionId: _s, pageSize: _p, poolId, ...rest } = params.value
  router.replace({
    query: { ...rest, ...(poolId ? { pool: poolId } : {}), ...(allSessions.value ? { all: '1' } : {}) },
  })
}
...
watch([qDebounced, status, attention, pool, allSessions], () => {
  page.value = 1
})
...
const columns = computed<ColumnDef<Row>[]>(() => [
    ...
    ? [
        {
          id: 'session',
          header: 'Session',
          cell: ({ row }) => `${row.original.program} · ${row.original.session}`,
  ...
  { accessorKey: 'health', header: 'Health' },
  {
    ...
    meta: {
      cellClass: (r) => [
        'tabular-nums',
        r.waiversSigned < r.waiversRequired ? 'font-medium text-destructive' : 'text-muted-foreground',
      ],
    },
  },
  {
    accessorKey: 'balance',
    header: sortable('Balance'),
    enableSorting: true,
    sortDescFirst: true,
    cell: ({ row }) => (row.original.balance > 0 ? money(row.original.balance) : '—'),
    meta: {
      class: 'text-right',
      cellClass: (r) => ['tabular-nums', r.balance > 0 ? 'font-medium' : 'text-muted-foreground'],
    },
  },
])
</script>
  ...
  <div class="space-y-4">
    ...
    >
      ...
      <template #cell-health="{ row }">
        <StatusBadge
          :status="row.health"
          :label="row.healthMechanism === 'CampDoc' && row.health === 'Incomplete' ? 'CampDoc incomplete' : undefined"
        />
      </template>
    </DataTable>
