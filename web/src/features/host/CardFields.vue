...
<template>
  <fieldset class="space-y-4 rounded-lg border bg-muted/20 p-4" :disabled="disabled">
    ...
    <div class="space-y-2">
      <Label for="h-cc">Card number</Label>
      <Input
        id="h-cc"
        v-model="card.number"
        inputmode="numeric"
    ...
    <div class="grid grid-cols-3 gap-3">
      <div class="space-y-2">
        <Label for="h-exp">Expiry</Label>
        <Input id="h-exp" v-model="card.expiry" autocomplete="cc-exp" placeholder="MM / YY" />
      </div>
      <div class="space-y-2">
        <Label for="h-cvc">CVC</Label>
        <Input id="h-cvc" v-model="card.cvc" inputmode="numeric" autocomplete="cc-csc" placeholder="123" />
      </div>
      <div class="space-y-2">
        <Label for="h-zip">ZIP</Label>
        <Input id="h-zip" v-model="card.zip" inputmode="numeric" autocomplete="postal-code" placeholder="30303" />
      </div>
    </div>
  </fieldset>
