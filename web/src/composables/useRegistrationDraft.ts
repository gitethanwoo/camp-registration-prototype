...
function fresh(): RegistrationDraft {
  return {
    step: 0, selected: [], answers: {}, householdAnswers: {}, health: {}, agreed: {},
    signer: '', paymentOption: 'Deposit', discountCode: '', idempotencyKey: newKey(),
  }
}
...
export function useRegistrationDraft(sessionId: number) {
  ...
  catch { /* storage unavailable: draft lives in memory only */ }
  ...
  return {
    ...
    rotateKey: () => { draft.idempotencyKey = newKey() },
    clear: () => {
