...
import { ChevronsUpDown, LogOut, Search, Store } from '@lucide/vue'
...
const { session, initials, roleLabel, signOut } = useSession()

// Sidebar entries come from route meta, so feature slices add pages without editing this file.
const groups = computed(() => {
  ...
  return navGroups
    .map((label) => ({
      label,
      items: items.filter((i) => i.group === label).toSorted((a, b) => (a.order ?? 99) - (b.order ?? 99)),
    }))
      ...
      <SidebarHeader>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger as-child>
            <SidebarMenuButton
              size="lg"
              class="border bg-background data-[state=open]:bg-sidebar-accent"
              tooltip="Scope"
            >
              <div class="grid flex-1 text-left leading-tight">
                <span class="truncate text-xs text-muted-foreground"
                  >{{ current?.ministry.name ?? 'Loading…' }} ▸ {{ current?.program.name }}</span
                >
            ...
            <template v-for="m in ministries" :key="m.id">
              ...
              <template v-for="p in m.programs" :key="p.id">
                <DropdownMenuItem
                  ...
                  :key="s.id"
                  class="flex-col items-start gap-0 pl-4"
                  @select="choose(s.id)"
                >
                  <span>{{ p.name }} · {{ s.name }}</span>
                  <span class="text-xs text-muted-foreground"
                    >{{ date(s.startDate) }}{{ p.type !== 'Standard' ? ` · ${p.type}` : '' }}</span
                  >
                </DropdownMenuItem>
              </template>
              <DropdownMenuSeparator />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent side="top" align="start" class="w-60">
            ...
            <DropdownMenuItem as-child
              ><RouterLink to="/programs"><Store /> Guest site</RouterLink></DropdownMenuItem
            >
            <DropdownMenuSeparator />
          ...
          <BreadcrumbList class="flex-nowrap">
            <BreadcrumbItem class="hidden md:block"
              ><BreadcrumbLink as-child
                ><RouterLink to="/admin">{{ current?.ministry.name }}</RouterLink></BreadcrumbLink
              ></BreadcrumbItem
            >
            <BreadcrumbSeparator class="hidden md:block" />
            <BreadcrumbItem class="hidden sm:block"
              ><span class="truncate">{{ current?.program.name }} · {{ current?.session.name }}</span></BreadcrumbItem
            >
